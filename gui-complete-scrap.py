...
open_button = tk.Button(frame_input, text="Otevři stránku", command=lambda: open_page(get_full_url(url_entry.get(), via_param.get()), adults, children, children_ages, status_label, notebook, via_param.get()))
...
adults_spinbox = tk.Spinbox(frame_input, from_=1, to=16, width=5, command=lambda: update_adults(int(adults_spinbox.get())))
...
children_spinbox = tk.Spinbox(frame_input, from_=0, to=4, width=5, command=lambda: update_children(int(children_spinbox.get())))
...
for i in range(4):
    ...
    age_spinbox = tk.Spinbox(frame_input, from_=0, to=18, width=5, state='disabled', command=lambda i=i: update_child_age(i, int(age_spinboxes[i].get())))
...
description_label = tk.Label(frame_descriptions, text="", wraplength=600, justify="left")
